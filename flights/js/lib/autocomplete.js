...
export function attachAutocomplete(input) {
  ...
  input.addEventListener('input', async () => {
    ...
    function select(i){ const it=items[i]; input.value=`${it.name} (${it.code})`; input.dataset.code=it.code; menu.hidden=true; }
  });
  ...
  input.addEventListener('keydown', (e) => {
    ...
    else if (e.key === 'Enter') { if (idx >= 0) { const it = items[idx]; input.value = `${it.name} (${it.code})`; input.dataset.code = it.code; menu.hidden = true; e.preventDefault(); } }
