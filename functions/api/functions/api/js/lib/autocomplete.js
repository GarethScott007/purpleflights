...
export function attachAutocomplete(input) {
  ...
  input.addEventListener('input', async () => {
    ...
    items = (Array.isArray(list) ? list : [])
      .map(x => ({ name: x.name, code: x.code, country: x.country || '' }))
      .filter(x => x.code && x.name)
      .sort((a, b) => score(q, a) - score(q, b))
      .slice(0, 12);
    ...
    items.forEach((it, i) => {
      ...
      d.innerHTML = `<strong>${it.name}</strong> <span style="color:#6b7280">(${it.code}) • ${it.country}</span>`;
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') { idx = Math.min(idx + 1, items.length - 1); highlight(idx); e.preventDefault(); }
    else if (e.key === 'ArrowUp') { idx = Math.max(idx - 1, 0); highlight(idx); e.preventDefault(); }
    else if (e.key === 'Enter') { if (idx >= 0) { select(idx); e.preventDefault(); } }
  ...
  function highlight(i) {
    [...menu.children].forEach((c, k) => {
      c.style.background = k === i ? '#f3f4ff' : '#fff';
    });
    idx = i;
  }
  function select(i) {
